...
from orchestrate import cmd_body, cmd_list, cmd_read, cmd_read_all

HELP = """\
you-got-mail list [--page OFFSET] [--limit N]
you-got-mail read <id>
you-got-mail read-all
you-got-mail body <id>
you-got-mail accounts ...

The panel calls list, read, read-all, and body. Accounts are added in a terminal:
...
def main() -> None:
    ...
    if args[0] == "body":
        if len(args) < 2:
            die("usage: you-got-mail body <id>")
        cmd_body(args[1])
        return
    if args[0] == "accounts":
